...
This LLM Usage Policy is copied from
[the Rust project LLM Usage Policy](https://forge.rust-lang.org/policies/llm-usage.html)
and it is lightly modified to adapt it to the gr-satellites project.

This is a moderation policy for how LLMs are used in gr-satellites.
For additional information about the policy itself, see [the appendix](#appendix).

## Overview

Using LLMs while working on gr-satellites is conditionally allowed, when done with care.
...
- Documentation that is originally created by an LLM.
    ...
    - ℹ️ This includes user-facing output of CLI tools, such as CLI argument help and error messages.
         LLMs are conditionally allowed to assist with the *logic* surrounding these (see "Experiment: LLM-created code changes" below),
         but they must not be used to create the output text itself.
...
Pre-arranged, high-quality, well-tested, and well-reviewed code changes that are originally created by an LLM are allowed, **with disclosure**.
...
1. "Pre-arranged" means that a reviewer has communicated *ahead of time* that they are willing to review an LLM-created PR.
    ...
    - Authors are, of course, allowed to start work on a change before finding a reviewer.
      ...
      It is allowed, but discouraged, to open a LLM-created PR before finding a reviewer;
      we suggest instead opening a Github discussion to find a prospective reviewer.
      Such PRs still require disclosure.
2. "High-quality" means that it is held to at least the same standard as other code changes.
...
3. "Well-tested" means that you have covered all edge-cases that either you or the reviewer can think of.
...
4. "Well-reviewed" means the author and reviewer both commit to fully understanding the code.
...
There is not a consensus within the gr-satellites project—and likely never will be—about when/how/where it is acceptable to use AI-based tools.
Many members of the gr-satellites project and community find value in AI;
...
- Build an intentionally conservative policy that lets us maintain the standard of quality that gr-satellites is known for.
...
- Avoid making LLMs a requirement to contribute, to be inclusive of contributors who choose not to use them, and to avoid making gr-satellites "pay-to-play".
...
If you are not sure where something you would like to do falls in this policy, please reach out to talk about it.
...
The Code of Conduct applies to *all* conversations in the gr-satellites project.
